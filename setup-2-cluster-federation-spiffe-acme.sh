...
# 2-Cluster Federation Setup Script: https_spiffe + https_web (ACME)
# This script sets up SPIRE federation across 2 OpenShift clusters:
#   - Cluster 1: https_spiffe profile
#   - Cluster 2: https_web profile with ACME (Let's Encrypt)
#
# Prerequisites:
...
#   ./setup-2-cluster-federation-spiffe-acme.sh \
...
usage() {
    cat << EOF
Usage: $0 -k1 <kubeconfig1> -k2 <kubeconfig2> -c1 <cluster1-name> -c2 <cluster2-name> [-e <email>] --mode <apply|print-only>

Options:
    -k1, --kubeconfig1      Path to kubeconfig for cluster 1 (https_spiffe)
    -k2, --kubeconfig2      Path to kubeconfig for cluster 2 (https_web with ACME)
    -c1, --cluster1-name    Name for cluster 1
    -c2, --cluster2-name    Name for cluster 2
    -e,  --email           Email for ACME (Let's Encrypt) on cluster 2 (default: rausingh@redhat.com)
    -m,  --mode            Mode: 'apply' to apply changes, 'print-only' to print YAMLs
    -h,  --help            Display this help message

Example (Apply mode):
    $0 -k1 /path/to/kubeconfig1 -k2 /path/to/kubeconfig2 \\
       -c1 cluster1 -c2 cluster2 -e admin@example.com --mode apply
    ...
    bundleEndpoint:
      ...
      httpsWeb:
        acme:
          ...
          domainName: "federation.$APP_DOMAIN2"
          email: "$ACME_EMAIL"
          tosAccepted: "true"
    managedRoute: "true"
    federatesWith:
...
EOF
    ...
    if [[ "$MODE" == "print-only" ]]; then
        print_section "=== Cluster 2 ($CLUSTER2_NAME) - SPIRE Resources with ACME ==="
        echo "$yaml_content"
        echo ""
    else
        print_info "Deploying SPIRE resources on Cluster 2 ($CLUSTER2_NAME) with ACME..."
...
print_info "Profile: https_spiffe + https_web (ACME)"
...
if [[ "$MODE" == "print-only" ]]; then
    ...
    print_info "4. Wait for ACME certificate provisioning (cluster 2)"
    print_info "5. Fetch bundles manually using: curl -k https://federation.<trust-domain>"
    print_info "6. Apply ClusterFederatedTrustDomain resources (update trustDomainBundle with fetched bundles)"
    print_info ""
    ...
    echo ""
    echo "# Fetch bundle from Cluster 2 (after ACME cert is provisioned):"
...
EOF
    ...
    print_info "Note: ACME certificate provisioning may take a few minutes on cluster 2"
    ...
    print_info "  Cluster 2 ($CLUSTER2_NAME): $APP_DOMAIN2 (https_web with ACME)"
    print_info ""
    ...
    print_info ""
    print_info "Note: Cluster 2 uses ACME (Let's Encrypt) for TLS certificate"
    print_info ""
