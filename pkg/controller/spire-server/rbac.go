...
func (r *SpireServerReconciler) cleanupExternalCertRBAC(ctx context.Context) error {
	// Check if federation Route exists and uses externalCertificate
	route := &routev1.Route{}
	...
	if err == nil && route.Spec.TLS != nil && route.Spec.TLS.ExternalCertificate != nil {
		...
		r.log.Info("Federation Route with external certificate still exists, keeping RBAC resources")
		return nil
	} else if err != nil && !kerrors.IsNotFound(err) {
		// Unexpected error getting route
		r.log.Error(err, "failed to get federation route while checking RBAC cleanup")
		return err
	}

	// Safe to delete RBAC - either no Route exists or Route doesn't use external certificate
	r.log.Info("No federation Route using external certificate, cleaning up RBAC resources")
