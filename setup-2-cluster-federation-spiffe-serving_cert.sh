...
# 2-Cluster Federation Setup Script: https_spiffe + https_web (Serving Cert)
# This script sets up SPIRE federation across 2 OpenShift clusters:
#   - Cluster 1: https_spiffe profile
#   - Cluster 2: https_web profile with serving cert
#
# Prerequisites:
...
#   ./setup-2-cluster-federation-spiffe-serving_cert.sh \
...
Options:
    ...
    -k2, --kubeconfig2      Path to kubeconfig for cluster 2 (https_web with serving cert)
...
EOF
    ...
    if [[ "$MODE" == "print-only" ]]; then
        print_section "=== Cluster 2 ($CLUSTER2_NAME) - SPIRE Resources with Serving Cert ==="
        echo "$yaml_content"
        echo ""
    else
        print_info "Deploying SPIRE resources on Cluster 2 ($CLUSTER2_NAME) with serving cert..."
...
print_info "Profile: https_spiffe + https_web (Serving Cert)"
...
if [[ "$MODE" == "print-only" ]]; then
    ...
    print_info ""
    print_info "Note: Cluster 2 uses service CA for internal route-to-pod communication (automatic)"
...
EOF
    ...
    print_section "=== Optional: Add External TLS Certificate to Cluster 2 ==="
    echo "# If you want to add a Let's Encrypt certificate for cluster 2 federation endpoint:"
    echo ""
    echo "# Step 1: Install cert-manager operator on cluster 2"
    echo "# (Apply via console UI or oc command)"
    echo ""
    echo "# Step 2: Create ACME Issuer (Let's Encrypt)"
    cat <<'EOF'
...
EOF
    echo ""
    echo "# Step 3: Create Certificate CR"
    cat <<EOF
apiVersion: cert-manager.io/v1
kind: Certificate
metadata:
...
spec:
  ...
  commonName: federation.$APP_DOMAIN2
  dnsNames:
    - federation.$APP_DOMAIN2
  usages:
    - server auth
  issuerRef:
    kind: Issuer
...
EOF
    echo ""
    echo "# Step 4: Patch SpireServer to use external certificate"
    echo "# Run: ./patch-external-tls-cert.sh -k \$KUBECONFIG2 -s spire-server-federation-tls"
    echo ""
    ...
    print_info "  Cluster 2 ($CLUSTER2_NAME): $APP_DOMAIN2 (https_web with serving cert)"
    print_info ""
    ...
    print_info ""
    print_info "=========================================="
    print_info "Optional: Add External TLS Certificate"
    print_info "=========================================="
    print_info ""
    print_info "To add Let's Encrypt certificate for cluster 2 federation endpoint:"
    print_info ""
    print_info "1. Install cert-manager operator on cluster 2"
    print_info "2. Create ACME Issuer and Certificate CR (see script in print-only mode)"
    print_info "3. Run patch script: ./patch-external-tls-cert.sh -k $KUBECONFIG2 -s spire-server-federation-tls"
    print_info ""
